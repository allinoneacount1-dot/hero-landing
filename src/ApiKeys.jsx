import { useState } from "react";
import { Link } from "react-router-dom";
import { Twitter, Menu, X, Key, Copy, Check, Trash2, Shield, ExternalLink, Github, MessageSquare, Package, BookOpen, Zap, Infinity, UserCheck, Slash } from "lucide-react";
import WalletConnector from "./components/WalletConnector";
import { useWallet } from "@solana/wallet-adapter-react";
import { useToast } from "./components/ToastProvider";

const randHex = (n) => Array.from({ length: n }, () => Math.floor(Math.random() * 16).toString(16)).join("");

const TIERS = [
  {
    name: "Free",
    icon: Zap,
    price: "$0",
    reqs: "100 req/day",
    keys: "1 API key",
    endpoints: "Basic price endpoints",
    support: "Community support",
    accent: "text-white",
  },
  {
    name: "Builder",
    icon: Shield,
    price: "$49/mo",
    reqs: "10,000 req/day",
    keys: "5 API keys",
    endpoints: "All endpoints + supply chain data",
    support: "Email support",
    accent: "text-blue-400",
  },
  {
    name: "Enterprise",
    icon: Infinity,
    price: "Custom",
    reqs: "Unlimited",
    keys: "Custom keys",
    endpoints: "Dedicated RPC, SLA, white-label",
    support: "Custom pricing",
    accent: "text-yellow-400",
  },
];

const ENDPOINTS = [
  { method: "GET", path: "/v1/prices", desc: "Real-time material & crypto prices" },
  { method: "GET", path: "/v1/marketplace", desc: "Construction material listings" },
  { method: "GET", path: "/v1/supply-chain", desc: "Track shipment by hash" },
  { method: "GET", path: "/v1/certificates", desc: "Verify NFT certificate" },
  { method: "GET", path: "/v1/governance", desc: "DAO proposals & votes" },
  { method: "POST", path: "/v1/swap", desc: "Execute token swap" },
  { method: "POST", path: "/v1/stake", desc: "Stake CTKN tokens" },
];

const METHOD_C = {
  GET: "text-green-400 bg-green-400/10",
  POST: "text-blue-400 bg-blue-400/10",
};

export default function ApiKeys() {
  const { connected } = useWallet();
  const [menuOpen, setMenuOpen] = useState(false);
  const [keys, setKeys] = useState([]);
  const [copied, setCopied] = useState(null);
  const { showToast } = useToast();

  const generateKey = () => {
    const key = `ctk_live_${randHex(48)}`;
    setKeys((prev) => [
      {
        id: key,
        name: `API Key ${prev.length + 1}`,
        key,
        created: new Date(),
        lastUsed: null,
        status: "active",
      },
      ...prev,
    ]);
    showToast("API key generated", "success");
  };

  const revokeKey = (id) => {
    setKeys((prev) => prev.map((k) => (k.id === id ? { ...k, status: "revoked" } : k)));
    showToast("API key revoked", "error");
  };

  const copyKey = (key) => {
    navigator.clipboard.writeText(key);
    setCopied(key);
    setTimeout(() => setCopied(null), 2000);
  };

  return (
    <div className="relative min-h-screen w-full bg-black text-white overflow-hidden">
      <video autoPlay loop muted playsInline className="absolute inset-0 w-full h-full object-cover">
        <source src="https://cdn.sceneai.art/Hero%20Section%20Video/a8132a81-b526-4f91-8095-003ce931ecdd.mp4" type="video/mp4" />
      </video>
      <div className="absolute inset-0 bg-black/70" />

      <div className="relative z-20 flex flex-col min-h-screen">
        <nav className="flex items-center justify-between px-4 sm:px-6 lg:px-12 py-4">
          <Link to="/">
            <svg fill="currentColor" height="32" viewBox="0 0 145 48" width="96" xmlns="http://www.w3.org/2000/svg" className="text-white shrink-0">
              <g fill="currentColor">
                <path clipRule="evenodd" d="m15.2286 4.99951c-3.2154 0-6.18655 1.71539-7.79425 4.5l-5.7735 9.99999c-1.6076941 2.7846-1.607697 6.2154 0 9l5.7735 10c1.6077 2.7846 4.57885 4.5 7.79425 4.5h11.547c3.2154 0 6.1865-1.7154 7.7942-4.5l5.7735-10c1.6077-2.7846 1.6077-6.2154 0-9l-5.7735-9.99999c-1.6077-2.78461-4.5788-4.5-7.7942-4.5zm11.547 5.99999h-7.2169c-1.1547 0-1.8762 1.2499-1.298 2.2494 1.784 3.0838 3.5722 6.1653 5.3536 9.2506.5359.9282.5359 2.0718 0 3-1.7814 3.0854-3.5696 6.1668-5.3536 9.2506-.5782.9995.1433 2.2494 1.298 2.2494h7.2169c1.0718 0 2.0622-.5718 2.5981-1.5l5.7735-10c.5359-.9282.5359-2.0718 0-3l-5.7735-10c-.5359-.9282-1.5263-1.5-2.5981-1.5z" fillRule="evenodd"/>
                <path d="m66.983 20.526h-4.536c-.513-1.809-1.809-2.781-3.645-2.781-2.781 0-4.374 2.322-4.374 6.102 0 3.807 1.566 6.048 4.374 6.048 1.728 0 3.078-.918 3.591-2.646h4.59c-.945 4.05-4.212 6.183-8.127 6.183-5.481 0-8.856-3.645-8.856-9.585s3.375-9.639 8.91-9.639c3.942 0 7.29 2.133 8.073 6.318z"/>
                <path d="m75.1442 33.432c-4.401 0-7.128-2.916-7.128-7.695 0-4.941 2.808-7.722 7.128-7.722 4.401 0 7.155 2.97 7.155 7.722 0 4.914-2.835 7.695-7.155 7.695zm0-3.348c1.917 0 2.916-1.512 2.916-4.347 0-2.808-1.026-4.374-2.916-4.374s-2.889 1.539-2.889 4.374c0 2.808 1.026 4.347 2.889 4.347z"/>
                <path d="m83.9439 33v-14.526h4.1309v2.025c.918-1.728 2.3221-2.484 3.8071-2.484.594 0 1.134.162 1.431.459v3.483c-.486-.108-.999-.162-1.647-.162-2.484 0-3.5911 1.404-3.5911 3.699v7.506z"/>
                <path d="m107.851 28.221c-.756 3.348-3.456 5.211-7.02 5.211-4.5093 0-7.2903-2.916-7.2903-7.695 0-4.941 2.808-7.722 7.1283-7.722 4.347 0 7.074 2.889 7.074 7.641v.918h-9.9903c.216 2.322 1.296 3.564 3.0783 3.564 1.35 0 2.268-.594 2.7-1.917zm-7.182-6.912c-1.5393 0-2.5113.999-2.8353 2.889h5.6433c-.324-1.89-1.296-2.889-2.808-2.889z"/>
                <path d="m118.324 33.432c-5.697 0-9.207-3.672-9.207-9.585 0-5.94 3.51-9.639 9.207-9.639 5.67 0 9.18 3.699 9.18 9.639 0 5.913-3.51 9.585-9.18 9.585zm0-3.537c2.997 0 4.752-2.268 4.752-6.048s-1.755-6.102-4.752-6.102c-3.024 0-4.779 2.295-4.779 6.102 0 3.78 1.755 6.048 4.779 6.048z"/>
                <path d="m133.466 19.365c0 3.915 10.8.594 10.8 8.1 0 3.78-3.132 5.967-7.425 5.967-4.347 0-7.452-1.998-8.1-6.183h4.563c.351 1.809 1.62 2.808 3.564 2.808s2.97-.783 2.97-2.052c0-4.104-10.827-.972-10.827-8.235 0-3.078 2.565-5.562 7.074-5.562 3.807 0 7.101 1.809 7.668 6.048h-4.617c-.378-1.809-1.431-2.673-3.213-2.673-1.512 0-2.457.702-2.457 1.782z"/>
              </g>
            </svg>
          </Link>
          <div className="hidden md:flex items-center gap-6 text-[14px] font-normal text-white/70">
            <a href="https://x.com" target="_blank" rel="noopener noreferrer" className="hover:text-white transition-colors inline-flex items-center"><Twitter size={14} /></a>
            <WalletConnector />
          </div>
          <button className="md:hidden text-white" onClick={() => setMenuOpen((p) => !p)}>
            {menuOpen ? <X size={24} /> : <Menu size={24} />}
          </button>
        </nav>

        {menuOpen && (
          <div className="fixed inset-0 z-30 bg-black flex flex-col items-center justify-center gap-8 text-lg">
            <button className="absolute top-4 right-4 text-white" onClick={() => setMenuOpen(false)}><X size={28} /></button>
            <div onClick={() => setMenuOpen(false)}><WalletConnector /></div>
          </div>
        )}

        <main className="flex-1 px-4 sm:px-6 lg:px-12 py-6 overflow-y-auto">
          <div className="max-w-5xl mx-auto space-y-8">
            <div>
              <h1 className="text-2xl font-bold">API Platform</h1>
              <p className="text-white/50 text-sm">Build on ConstrToken — real-time construction data, Solana RPC, and market intelligence</p>
            </div>

            <div className="bg-white/5 border border-white/10 p-5">
              <h2 className="text-sm font-semibold mb-4 flex items-center gap-2"><Key size={14} /> API Key Generator</h2>
              {connected ? (
                <div className="space-y-4">
                  <button
                    onClick={generateKey}
                    className="bg-white text-black px-4 py-2 text-sm font-medium hover:scale-105 active:scale-95 transition-transform rounded-none"
                  >
                    Generate API Key
                  </button>

                  {keys.length > 0 && (
                    <div className="space-y-2">
                      {keys.map((k) => (
                        <div key={k.id} className="bg-black/40 border border-white/10 p-3 flex items-center justify-between gap-3">
                          <div className="flex items-center gap-3 min-w-0">
                            <Key size={14} className="text-white/40 shrink-0" />
                            <div className="min-w-0">
                              <p className="text-xs font-medium">{k.name}</p>
                              <p className="text-[11px] font-mono text-white/40 truncate">
                                {k.key.slice(0, 12)}...{k.key.slice(-4)}
                              </p>
                              <p className="text-[10px] text-white/30">
                                Created {k.created.toLocaleDateString()} · {k.lastUsed ? `Last used ${k.lastUsed.toLocaleDateString()}` : "Never used"}
                              </p>
                            </div>
                          </div>
                          <div className="flex items-center gap-2 shrink-0">
                            <span className={`text-[10px] px-2 py-0.5 rounded ${k.status === "active" ? "text-green-400 bg-green-400/10" : "text-red-400 bg-red-400/10"}`}>
                              {k.status === "active" ? "Active" : "Revoked"}
                            </span>
                            <button
                              onClick={() => copyKey(k.key)}
                              className="text-white/40 hover:text-white transition-colors"
                              title="Copy key"
                            >
                              {copied === k.key ? <Check size={14} className="text-green-400" /> : <Copy size={14} />}
                            </button>
                            {k.status === "active" && (
                              <button
                                onClick={() => revokeKey(k.id)}
                                className="text-white/40 hover:text-red-400 transition-colors"
                                title="Revoke key"
                              >
                                <Slash size={14} />
                              </button>
                            )}
                          </div>
                        </div>
                      ))}
                    </div>
                  )}
                </div>
              ) : (
                <p className="text-sm text-white/40">Connect wallet to generate API keys</p>
              )}
            </div>

            <div>
              <h2 className="text-sm font-semibold mb-4">API Tiers</h2>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                {TIERS.map((tier) => (
                  <div key={tier.name} className="bg-white/5 border border-white/10 p-5 flex flex-col">
                    <div className="flex items-center gap-2 mb-3">
                      <tier.icon size={16} className={tier.accent} />
                      <p className="text-sm font-semibold">{tier.name}</p>
                    </div>
                    <p className="text-xl font-bold mb-4">{tier.price}</p>
                    <div className="space-y-2 text-xs text-white/60 flex-1">
                      <p className="flex items-center gap-2"><Zap size={12} className="text-white/30" />{tier.reqs}</p>
                      <p className="flex items-center gap-2"><Key size={12} className="text-white/30" />{tier.keys}</p>
                      <p className="flex items-center gap-2"><Shield size={12} className="text-white/30" />{tier.endpoints}</p>
                      <p className="flex items-center gap-2"><UserCheck size={12} className="text-white/30" />{tier.support}</p>
                    </div>
                    <button className="w-full mt-4 text-xs py-2 bg-white/5 border border-white/10 hover:bg-white/10 transition-colors">
                      {tier.price === "Custom" ? "Contact Sales" : "Get Started"}
                    </button>
                  </div>
                ))}
              </div>
            </div>

            <div>
              <h2 className="text-sm font-semibold mb-4">REST API Endpoints</h2>
              <div className="space-y-1">
                {ENDPOINTS.map((ep) => (
                  <div key={ep.path} className="bg-white/5 border border-white/10 p-3 flex items-center gap-3">
                    <span className={`text-[10px] font-mono px-2 py-0.5 rounded ${METHOD_C[ep.method]}`}>{ep.method}</span>
                    <span className="font-mono text-xs text-white/70">{ep.path}</span>
                    <span className="text-xs text-white/40 ml-auto">{ep.desc}</span>
                  </div>
                ))}
              </div>
            </div>

            <div className="bg-white/5 border border-white/10 p-5">
              <h2 className="text-sm font-semibold mb-3">Quick Start</h2>
              <pre className="bg-black/60 border border-white/10 p-4 text-[12px] font-mono overflow-x-auto">
{`const API_KEY = "ctk_live_your_key_here";
const res = await fetch("https://api.constrtoken.io/v1/prices", {
  headers: { "X-API-Key": API_KEY }
});
const data = await res.json();
console.log(data.steel, data.solana, data.cement);`}
              </pre>
            </div>

            <div className="bg-white/5 border border-white/10 p-5">
              <h2 className="text-sm font-semibold mb-4">Documentation & SDK</h2>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                <a href="#" className="flex items-center gap-2 text-xs py-2 px-3 bg-white/5 border border-white/10 hover:bg-white/10 transition-colors">
                  <BookOpen size={14} className="text-white/40" /> Docs
                </a>
                <a href="#" className="flex items-center gap-2 text-xs py-2 px-3 bg-white/5 border border-white/10 hover:bg-white/10 transition-colors">
                  <Github size={14} className="text-white/40" /> GitHub
                </a>
                <a href="#" className="flex items-center gap-2 text-xs py-2 px-3 bg-white/5 border border-white/10 hover:bg-white/10 transition-colors">
                  <MessageSquare size={14} className="text-white/40" /> Discord
                </a>
                <a href="#" className="flex items-center gap-2 text-xs py-2 px-3 bg-white/5 border border-white/10 hover:bg-white/10 transition-colors">
                  <Package size={14} className="text-white/40" /> npm
                </a>
              </div>
            </div>
          </div>
        </main>
      </div>
    </div>
  );
}
