...
const stats = [
  ...
  { label: "Materials Delivered", value: "8,421", icon: Truck },
  { label: "Inspections Passed", value: "100%", icon: ClipboardCheck },
  { label: "Budget Utilization", value: "$2.4M", icon: BarChart3 },
];
      ...
      <div className="relative z-20 flex flex-col min-h-screen">
        ...
        {menuOpen && (
          <div className="fixed inset-0 z-30 bg-black flex flex-col items-center justify-center gap-8 text-lg">
            <button
              className="absolute top-4 right-4 sm:top-6 sm:right-6 lg:top-8 lg:right-12 text-white"
              onClick={() => setMenuOpen(false)}
              aria-label="Close menu"
            >
              <X size={28} />
            </button>
            {["Home", "Dashboard", "Analytics", "Reports"].map((item) => (
              <Link
                key={item}
        ...
        <main className="flex-1 px-4 sm:px-6 lg:px-12 py-8">
          <div className="max-w-6xl mx-auto">
            ...
            <p className="text-white/60 mb-10 text-sm">Overview of your construction projects</p>

            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
              {stats.map((s) => {
                const Icon = s.icon;
                return (
                  <div key={s.label} className="bg-white/5 border border-white/10 p-5">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
              <div className="bg-white/5 border border-white/10 p-5">
                ...
                <div className="space-y-3">
                  {["Steel Beams - 200 units", "Concrete Mix - 500 bags", "Plywood Sheets - 150 pcs"].map((order, i) => (
                    <div key={i} className="flex items-center justify-between text-sm">
                      <span className="text-white/70">{order}</span>
                      <span className={`text-xs px-2 py-0.5 ${i === 0 ? "text-yellow-300 bg-yellow-300/10" : i === 1 ? "text-green-300 bg-green-300/10" : "text-blue-300 bg-blue-300/10"}`}>
                        {i === 0 ? "In Progress" : i === 1 ? "Delivered" : "Pending"}
                      </span>
                    </div>
                  ))}
                </div>
              ...
              <div className="bg-white/5 border border-white/10 p-5">
                <h3 className="text-sm font-medium mb-4">Wallet Activity</h3>
                <div className="flex flex-col items-center justify-center py-6 text-center">
                  <Wallet size={32} className="text-white/30 mb-3" />
                  <p className="text-white/50 text-sm">Connect your wallet to view</p>
                  <p className="text-white/30 text-xs mt-1">on-chain transaction history</p>
                </div>
              </div>
            </div>
          </div>
